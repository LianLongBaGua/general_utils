...
def get_desired_pos(df, lag=50, multiplier=10):
    ...
    dpos_change_signal = pd.qcut(df['desired_pos_change'], 5, ['strong sell', 'sell', 'meh', 'buy', 'strong buy'])
    desired_pos_rolling = df['desired_pos_change'].rolling(lag, min_periods=1).sum().apply(int)
    net_pos_signal = np.where(df['desired_pos_rolling'] > 0, 'long hold', 'short hold')
    df.drop(columns=[f'{lag}m_ret'], inplace=True)

    return desired_pos_change, dpos_change_signal, desired_pos_rolling, net_pos_signal